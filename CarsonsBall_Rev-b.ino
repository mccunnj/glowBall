...
void handleSave() {
  preferences.begin("settings", false);  // Open preferences before writing
  ...
  if (server.hasArg("mode")) preferences.putInt("mode", server.arg("mode").toInt());
  if (server.hasArg("brightness")) preferences.putInt("brightnessNV", lightBright);
  if (server.hasArg("primary")) preferences.putString("primary", server.arg("primary"));
  if (server.hasArg("secondary")) preferences.putString("secondary", server.arg("secondary"));
  if (server.hasArg("tertiary")) preferences.putString("tertiary", server.arg("tertiary"));
  ...
  preferences.end();  // Close immediately after writing
